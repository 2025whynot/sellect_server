...
@Slf4j
public class AutoCompleteKeywordBatchV4 {
    ...
    public Step processAutoCompleteKeywordStep() throws Exception {
        ...
        return new StepBuilder("processAutoCompleteKeywordStep", jobRepository)
            .<KeywordCount, AutoCompleteKeyword>chunk(CHUNK_SIZE, transactionManager)
            .reader(keywordCountItemReader(lastProcessedId, maxId))
            .processor(keywordCountProcessor())
    ...
    @Bean
    @StepScope
    public JdbcPagingItemReader<KeywordCount> keywordCountItemReader(
        @Value("#{stepExecutionContext['lastProcessedId']}") Long lastProcessedId,
        @Value("#{stepExecutionContext['maxId']}") Long maxId) throws Exception {

        log.info("ItemReader initialized with lastProcessedId={}, maxId={}", lastProcessedId, maxId);

        Map<String, Object> parameterValues = new HashMap<>();
        parameterValues.put("startId", lastProcessedId + 1);
        parameterValues.put("endId", maxId);

        return new JdbcPagingItemReaderBuilder<KeywordCount>()
            .name("keywordCountItemReader")
            .dataSource(dataSource)
            .queryProvider(createKeywordCountQueryProvider())
            .parameterValues(parameterValues)
            .pageSize(CHUNK_SIZE)
            .rowMapper(new BeanPropertyRowMapper<>(KeywordCount.class))
            .build();
    }

    private PagingQueryProvider createKeywordCountQueryProvider() throws Exception {
        SqlPagingQueryProviderFactoryBean queryProvider = new SqlPagingQueryProviderFactoryBean();
        queryProvider.setDataSource(dataSource);
        queryProvider.setSelectClause("SELECT keyword, COUNT(*) as frequency");
    ...
    @Bean
    @StepScope
    public ItemProcessor<KeywordCount, AutoCompleteKeyword> keywordCountProcessor() {
        return item -> {
